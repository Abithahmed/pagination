...
function displayTable(data, table, rowsPerPage, page) {
  ...
  for (let i = 0; i < paginatedItems.length; i++) {
      ...
      row.insertCell().innerText = paginatedItems[i].id;
      row.insertCell().innerText = paginatedItems[i].name;
      row.insertCell().innerText = paginatedItems[i].email;
  }
}

function setupPagination(data, pagination, rowsPerPage) {
  pagination.innerHTML = "";
  const pageCount = Math.ceil(data.length / rowsPerPage);

  const firstButton = paginationButton("First", () => goToPage(1));
  const prevButton = paginationButton("Previous", () => goToPage(currentPage - 1));
  const nextButton = paginationButton("Next", () => goToPage(currentPage + 1));
  const lastButton = paginationButton("Last", () => goToPage(pageCount));

  pagination.appendChild(firstButton);
  pagination.appendChild(prevButton);

  for (let i = 1; i <= pageCount; i++) {
      const btn = paginationButton(i, () => goToPage(i));
  ...
  updateButtonStates(firstButton, prevButton, nextButton, lastButton, pageCount);
}
...
function goToPage(page) {
  ...
  currentPage = page;
  displayTable(data, document.getElementById("data-table"), rowsPerPage, currentPage);

  const buttons = document.querySelectorAll(".pagination button");
  buttons.forEach(button => button.classList.remove("active"));
  buttons[currentPage + 1].classList.add("active"); // +1 to account for "First" and "Previous" buttons
...
function updateButtonStates(firstButton, prevButton, nextButton, lastButton, pageCount) {
